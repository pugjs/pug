...
// @ts-expect-error TS(2451): Cannot redeclare block-scoped variable 'compileAtt... Remove this comment to see the full error message
...
function generateCode(ast: any, options: any) {
...
function Compiler(this: any, node: any, options: any) {
...
Compiler.prototype = {
	...
	error(message: any, code: any, node: any) {
	...
	compile() {
		...
		if (this.options.self) {
			...
			js = addWith(
				...
				js,
				globals.concat(
					this.runtimeFunctionsUsed.map((name: any) => {
						return `pug_${name}`;
					})
				)
			);
		...
		if (this.debug) {
			...
			js =
				`${"var pug_debug_filename, pug_debug_line;" + "try {"}${js}} catch (err) {${
		...
		}(locals) {var pug_html = "", pug_mixins = {}, pug_interp;${js};return pug_html;}`;
	},
	...
	prettyIndent(offset: any, newline: any) {
		offset = offset || 0;
		newline = newline ? "\n" : "";
	...
	// @ts-expect-error TS(6133): 'block' is declared but its value is never read.
	visitMixinBlock(block: any) {
	...
	visitMixin(mixin: any) {
		...
		if (mixin.call) {
			...
			let rest;
			if (args.length && args[args.length - 1].trim().startsWith("...")) {
				rest = args
					.pop()
					.trim()
					.replace(/^\.\.\./, "");
	...
	visitTag(tag: any, interpolated: any) {
		this.indents++;
		const name = tag.name,
			pp = this.pp,
			self = this;
	...
	visitEach(each: any) {
		...
		this.buf.push(
			`${
				"" + "      for (var "
			}${indexVarName} = 0, $$l = $$obj.length; ${indexVarName} < $$l; ${indexVarName}++) {\n` +
	...
	visitAttributes(attrs: any, attributeBlocks: any) {
	...
	attributeBlocks(attributeBlocks: any) {
		return (
			attributeBlocks &&
			attributeBlocks.slice().map((attrBlock: any) => {
