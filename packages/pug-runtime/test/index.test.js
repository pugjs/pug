...
addTest('attr', function(attr) {
  ...
  expect(attr('key', { foo: 'bar' }, false, true)).toBe(
    ' key=\'{"foo":"bar"}\''
  );
...
addTest('attrs', function(attrs) {
  ...
  expect(attrs({ class: ['foo', { bar: true }] }, true)).toBe(
    ' class="foo bar"'
  );
  expect(attrs({ class: ['foo', { bar: true }] }, false)).toBe(
    ' class="foo bar"'
  );
...
addTest('merge', function(merge) {
  expect(merge({ foo: 'bar' }, { baz: 'bash' })).toEqual({
    foo: 'bar',
    baz: 'bash'
  });
  ...
  expect(merge({ class: null }, { class: ['bar'] })).toEqual({
    class: ['bar']
  });
