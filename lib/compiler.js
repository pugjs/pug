...
Compiler.prototype = {
      ...
      if (match) {
        ...
        if (match[1]) { // escape
          ...
          try {
            ...
            var code = nullDefault(range.src);
            if ('!' != match[2]) code = escape(code);
          } catch (ex) {
            throw ex;
  ...
  bufferExpression: function (src) {
    if (isConstant(src)) {
      return this.buffer(Function('return ' + src)() + '');
    }
    if (this.lastBufferedIdx == this.buf.length) {
    ...
    attrs.forEach(function(attr){
      ...
      if (attr.name == 'class') {
        ...
        if (isConstant(attr.val)) {
          var val = Function('return ' + attr.val)();
          if (attr.escaped && !(typeof val === 'boolean' || val == null) &&
                              !(0 == attr.name.indexOf('data') && 'string' != typeof val)) {
            val = runtime.escape(val);
          } else if (attr.escaped) {
            escaped[attr.name] = attr.escaped;
          }
          pair = "'" + attr.name + "':" + JSON.stringify(val);
        } else {
...
function nullDefault(expression) {
  if (isConstant(expression)) {
    var val = Function('return ' + expression)();
    if (val == null) val = '';
    return JSON.stringify(val);
  } else {
    return '((jade.interp = ' + expression + ') == null ? "" : jade.interp)';
  }
}
function escape(expression) {
  if (isConstant(expression)) {
    var val = runtime.escape(Function('return ' + expression)());
    return JSON.stringify(val);
  } else {
    return 'jade.escape(' + expression + ')';
  }
}
/**
...
function isConstant(val){
  ...
  // Check for brackted literals
  var matches;
  if (matches = /^\((.*)\)$/.exec(val))
    return isConstant(matches[1]);

  // Check for arithmetic of literals
  var matches;
  if (matches = /^(.*)[\+\-\/\*\=\!\|\&]{1,2}(.*)$/.exec(val))
    return isConstant(val[1]) && isConstant(val[2]);
