...
Compiler.prototype = {
  ...
  visitAttributes: function(attrs){
    ...
    if(isConstant) {
      this.buf.push("buf.push('" + toLiteral(staticAttrs(this.terse, attrs)) + "')");
      return;
    }
...
function checkConstant(val) {
  return (typeof val != 'string') || /^('[^']*'|"[^"]*"|\d*|0x[\da-f]*|true|false|undefined|null)$/i.test(val);
}
...
function staticAttrs(terse, attrs){
  ...
  if (attrs.length) {
    attrs.forEach(function(attr){
      var val = stripConstant(attr.val);
      if (attr.name == 'class') {
        classes.push(val);
      } else {
        obj[attr.name] = val;
