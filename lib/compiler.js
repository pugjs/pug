...
Compiler.prototype = {
  ...
  buffer: function (str, interpolate) {
    ...
    var pushMerge = (!this.options.streaming || this.lastBufferedType === 'text');
    if (pushMerge && this.lastBufferedIdx == this.buf.length) {
  ...
  bufferExpression: function (src) {
    ...
    var pushMerge = (!this.options.streaming || this.lastBufferedType === 'code');
    if (pushMerge && this.lastBufferedIdx == this.buf.length) {
  ...
  prettyIndent: function(offset, newline){
    ...
    if (this.parentIndents) {
      if (this.options.streaming) {
        // push must follow the stream.Readable.push API; only one chunk
        this.buf.push("buf.push(jade_indent.join(''));");
      } else {
  ...
  visitMixin: function(mixin){
    ...
    if (mixin.call) {
      ...
      this.buf.push(name + (this.options.streaming ? (' = function*(' + args + '){') : (' = function(' + args + '){')));
  ...
  visitEach: function(each){
    this.buf.push(''
      + '// iterate ' + each.obj + '\n'
      + (this.options.streaming ? ';yield* (function*(){\n' : ';(function(){\n')
      + '  var $$obj = ' + each.obj + ';\n'
