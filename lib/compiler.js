  ...
  visitMixin: function(mixin){
    ...
    if (mixin.call) {
      ...
      if (!block && !attrs.length) {
        // Shortcut if conventional mixin is used
        this.buf.push(name + '(' + args + ');');
      } else {
      
        if (block) {
          if (block.inherits)
            this.buf.push('var __block = this.block;');
          
          this.buf.push(name + '.call({block: function(){');
          if (block.inherits)
            this.buf.push('__block();');
        ...
        if (attrs.length) {
          ...
          if (val.inherits) {
            this.buf.push('attributes: this.attributes, escaped: this.escaped');
          } else {
