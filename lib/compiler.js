...
Compiler.prototype = {
  ...
  visitBlock: function(block){
    ...
    if (this.parentIndents && block.mode) {
      ...
      this.buf.push('var loop = loop || undefined;');
      this.buf.push('block && block(loop);');
  ...
  visitMixin: function(mixin){
    ...
    if (mixin.call) {
      ...
      if (block || attrs.length) {
        ...
        if (block) {
          this.buf.push('block: function(loop){');
  ...
  visitFor: function(forNode){
    this.buf.push(''
      + ';(function(){\n'
      + '   var loop = jade.loop(' + forNode.length + ');');

    this.buf.push(''
      + '    for (' + forNode.condition + ') {\n');

    this.visit(forNode.block);

    this.buf.push('   loop.iterate();'
      + '    }\n}).call(this);\n');
  },
