...
Compiler.prototype = {
  ...
  visit: function(node){
    ...
    if (debug) {
      this.buf.push('jade.debug[__jadeDebuggerID__].unshift({ lineno: ' + node.line
        + ', filename: ' + (node.filename
          ? JSON.stringify(node.filename)
          : 'jade.debug[__jadeDebuggerID__][0].filename')
        + ' });');
    }
    ...
    if (debug) this.buf.push('jade.debug[__jadeDebuggerID__].shift();');
  },
