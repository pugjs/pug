...
Compiler.prototype = {
  ...
  visitAttributes: function(attrs, attributeBlocks){
    if (attributeBlocks.length) {
      ...
      this.bufferExpression('jade.attrs(jade.merge([' + attributeBlocks.join(',') + ']), ' + stringify(this.terse) + ')');
    } else if (attrs.length) {
  ...
  attrs: function(attrs, buffer){
    ...
    attrs.forEach(function(attr){
      ...
      if (key === 'class') {
        ...
        if (buffer) {
          this.bufferExpression('jade.attr("' + key + '", ' + attr.val + ', ' + stringify(escaped) + ', ' + stringify(this.terse) + ')');
        } else {
          var val = attr.val;
          if (key === 'style') {
            val = 'jade.style(' + val + ')';
    ...
    if (buffer) {
      if (classes.every(isConstant)) {
        ...
        this.bufferExpression('jade.cls([' + classes.join(',') + '], ' + stringify(classEscaping) + ')');
      }
    } else if (classes.length) {
      if (classes.every(isConstant)) {
        classes = stringify(runtime.joinClasses(classes.map(toConstant).map(runtime.joinClasses).map(function (cls, i) {
          return classEscaping[i] ? runtime.escape(cls) : cls;
        })));
      } else {
        classes = '(jade_interp = ' + stringify(classEscaping) + ',' +
