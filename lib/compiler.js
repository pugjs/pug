...
Compiler.prototype = {
  ...
  visitBlock: function(block){
    ...
    if (pp && len > 1 && !escape && block.nodes[0].type === 'Text' && block.nodes[1].type === 'Text')
      this.prettyIndent(1, true);

    for (var i = 0; i < len; ++i) {
      // Pretty print text
      if (pp && i > 0 && !escape && block.nodes[i].type === 'Text' && block.nodes[i-1].type === 'Text')
...
function tagCanInline(tag) {
  ...
  function isInline(node){
    ...
    return node.type === 'Text' || node.isInline;
  }
  ...
  if (tag.block.nodes.every(isInline)) {
