...
function parse(str, options, parseInfo){
  try {
    // Parse
    ...
    var compiler = new (options.compiler || Compiler)(parser.parse(), options)
      , js = compiler.compile();
    if (parseInfo) {
      parseInfo.dependencies = parser.dependencies;
    }
...
exports.compile = function(str, options, parseInfo){
  var options = options || {}
  ...
  if (options.compileDebug !== false) {
    fn = [
      ...
      , 'try {'
      , parse(str, options, parseInfo)
      , '} catch (err) {'
      , '  rethrow(err, __jade[0].filename, __jade[0].lineno);'
      , '}'
    ].join('\n');
  } else {
    fn = parse(str, options, parseInfo);
  }
