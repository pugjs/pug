...
Parser.prototype = {
  ...
  parseSpacedText: function(){
    var tok = this.expect('spaced-text');
    var tokens = this.parseInlineTagsInText(tok.val);
    if (tokens[0].isText) {
      tokens[0].val = '\n' + tokens[0].val;
    } else {
      tokens.unshift(new nodes.Text('\n'));
    }
    if (tokens[tokens.length-1].isText) {
      tokens[tokens.length-1].val += '\n';
    } else {
      tokens.push(new nodes.Text('\n'));
    }
    if (tokens.length === 1) return tokens[0];
    var node = new nodes.Block;
    for (var i = 0; i < tokens.length; i++) {
