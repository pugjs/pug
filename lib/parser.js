...
Parser.prototype = {
  ...
  parseTextBlock: function(){
    ...
    block.line = this.line();
    var body = [];
    var initialIndent = this.expect('indent').val;
    var indents = [initialIndent];
    var indentText = '';
    while ('outdent' != this.peek().type || indentText.length) {
      switch (this.peek().type) {
        case 'newline':
          this.advance();
          break;
        case 'indent':
          indents.push(this.expect('indent').val);
          indentText = Array((indents[indents.length - 1] - initialIndent) + 1).join(' ');
          break;
        case 'outdent':
          this.expect('outdent');
          indents.pop();
          ...
          break;
        default:
          body.push(indentText + this.advance().val);
      }
    }
    ...
    block.nodes = body.reduce(function (accumulator, text) {
