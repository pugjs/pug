...
Parser.prototype = {
  ...
  parseExtends: function(){
    var fs = require('fs');
    /**
    *Dynamic layout extends
    */
    var layout = this.expect('extends').val.trim();
    /**
     * Seacrh variable (? i dont know where is it...)
     */
    for(var vars in this.options) {
        var regexp = new RegExp('='+vars,'g');
        if(regexp.test(layout))
            layout = layout.replace(regexp,this.options[vars]);
    }
    var path = this.resolvePath(layout, 'extends');
