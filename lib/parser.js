...
Parser.prototype = {
  ...
  parseInlineFilter: function(){
    var block = new nodes.Block
      , tok = this.expect('::')
      , filter = this.expect('tag')
      , attrs = this.accept('attrs');

    block.push(this.parseText());
    var node = new nodes.Filter(filter.val, block, attrs && attrs.attrs);
    node.line = this.line();
    return node;
  },
  ...
  tag: function(tag){
    ...
    switch (this.peek().type) {
      ...
      case '::':
        tag.block  = new nodes.Block;
        tag.block.push(this.parseInlineFilter());
        break;
    }
