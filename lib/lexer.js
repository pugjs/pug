...
Lexer.prototype = {
    ...
    if (captures = /^\+([-\w]+)/.exec(this.input)) {
      ...
      if (captures = /^ *\((.*?)\)/.exec(this.input)) {
        if (!/^ *[-\w]+ *=/.test(captures[1])) {
          ...
          tok.args = captures[1]
             .split(',')
             .map(function(arg){return interpolate(arg)})
             .join(",");
        }
      }
  ...
  code: function() {
    ...
    if (captures = /^(!?=|-)([^\n]+)/.exec(this.input)) {
      ...
      captures[1] = interpolate(captures[2]);
...
var interpolate = function(str, quote){
