...
Lexer.prototype = {
  ...
  for: function() {
    var captures;
    if (captures = /^(?:- *)?(?:for\b) +(?: *\((.*)\))?/.exec(this.input)) {
      if (captures[1].indexOf('@') === -1) return;
      this.consume(captures[0].length);
      var length = /@([^;,]*)/.exec(captures[1])[1];
      var condition = captures[1].replace('@', '');
      var tok = this.tok('for', 'for (' + condition + ')');
