...
Lexer.prototype = {
  ...
  tag: function() {
    ...
    if (captures = /^(\w[-:\w]*)(\/?)/.exec(this.input)) {
      this.consume(captures[0].length);
      var tok, name = captures[1], filterCaptures;
      if (filterCaptures = /(\w[-:\w]*)::(\w[-\w]*)/.exec(name)) {
        name = filterCaptures[1]
        var filter = filterCaptures[2]
        tok = this.tok('tag', name);
        this.defer(this.tok('::'));
        this.defer(this.tok('tag', filter));
