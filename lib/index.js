...
function compileBody(str, options){
  var debug_sources = {};
  if (options.compileDebug) debug_sources[options.filename] = str;
  var dependencies = [];
  try {
    var ast = load.string(str, options.filename, {
      ...
      read: function (filename) {
        ...
        if (options.compileDebug) debug_sources[filename] = str;
        return str;
      }
    });
    ast = link(ast);
    ...
    // Compile
    var js = generateCode(ast, options);
  ...
  globals.push('jade_debug_filename');
  globals.push('jade_debug_line');
  globals.push('buf');
  var body = ''
    + 'var buf = [];\n'
    + 'var jade_mixins = {};\n'
    + 'var jade_interp;\n'
    + (options.compileDebug ? 'var jade_debug_sources = ' + stringify(debug_sources) + ';\n' : '')
    + (options.self
      ? 'var self = locals || {};\n' + js
      : addWith('locals || {}', '\n' + js, globals)) + ';'
    + 'return buf.join("");';
...
exports.compile = function(str, options){
  ...
  var parsed = compileBody(str, options);
  if (options.compileDebug !== false) {
    fn = [
      ...
      , parsed.body
      , '} catch (err) {'
      , '  jade.rethrow(err, jade_debug_filename, jade_debug_line' + (options.compileDebug === true ? ', jade_debug_sources[jade_debug_filename]' : '') + ');'
      , '}'
    ].join('\n');
  } else {
    fn = parsed.body;
...
exports.compileClientWithDependenciesTracked = function(str, options){
  ...
  var filename = options.filename ? stringify(options.filename) : 'undefined';
  var fn;
  var externalRuntime = options.externalRuntime || false;
  options.inlineRuntimeFunctions = !externalRuntime;
  ...
  if (options.compileDebug) {
    fn = [
      ...
      , parsed.body
      , '} catch (err) {'
      , '  ' + (externalRuntime ? 'jade.rethrow' : 'jade_rethrow') + '(err, jade_debug_filename, jade_debug_line, ' + stringify(str) + ');'
      , '}'
    ].join('\n');
  } else {
