...
describe('pug', function(){
  ...
  describe('.properties', function(){
    it('should have exports', function(){
      assert.equal('object', typeof pug.selfClosing, 'exports.selfClosing missing');
      assert.equal('object', typeof pug.doctypes, 'exports.doctypes missing');
      assert.equal('function', typeof pug.filters, 'exports.filters missing');
      assert.equal('object', typeof pug.utils, 'exports.utils missing');
      assert.equal('function', typeof pug.Compiler, 'exports.Compiler missing');
    });
  });
  ...
  describe('unit tests with .render()', function(){
    ...
    it('should support tags', function(){
      ...
      assert.equal('<div id="foo" class="bar"></div>', pug.render('#foo.bar'));
      assert.equal('<div id="foo" class="bar"></div>', pug.render('.bar#foo'));
      assert.equal('<div id="foo" class="bar"></div>', pug.render('div#foo(class="bar")'));
      assert.equal('<div id="foo" class="bar"></div>', pug.render('div(class="bar")#foo'));
      assert.equal('<div id="bar" class="foo"></div>', pug.render('div(id="bar").foo'));
    ...
    it('should support attrs', function(){
      ...
      assert.equal('<p data-dynamic="true" class="name"></p>', pug.render('p("class"= "name", "data-dynamic"= "true")'));
      assert.equal('<p data-dynamic="true"></p>', pug.render('p(\'data-dynamic\'= "true")'));
      assert.equal('<p data-dynamic="true" class="name"></p>', pug.render('p(\'class\'= "name", \'data-dynamic\'= "true")'));
      assert.equal('<p data-dynamic="true" yay="yay" class="name"></p>', pug.render('p(\'class\'= "name", \'data-dynamic\'= "true", yay)'));
    ...
    it('should support attr interpolation', function(){
      ...
      assert.equal('<a href="/user/12">tj</a>'
        , pug.render("a(href='/user/#{id}') #{name}", { name: 'tj', id: 12 }));

      assert.equal('<a href="/user/12-tj">tj</a>'
        , pug.render("a(href='/user/#{id}-#{name}') #{name}", { name: 'tj', id: 12 }));

      assert.equal('<a href="/user/&lt;script&gt;">tj</a>'
        , pug.render("a(href='/user/#{id}') #{name}", { name: 'tj', id: '<script>' }));

      // Test double quote interpolation
      assert.equal('<a href="/user/13">ds</a>'
        , pug.render('a(href="/user/#{id}") #{name}', { name: 'ds', id: 13 }));

      assert.equal('<a href="/user/13-ds">ds</a>'
        , pug.render('a(href="/user/#{id}-#{name}") #{name}', { name: 'ds', id: 13 }));

      assert.equal('<a href="/user/&lt;script&gt;">ds</a>'
        , pug.render('a(href="/user/#{id}") #{name}', { name: 'ds', id: '<script>' }));

      // Test escaping the interpolation
      assert.equal('<a href="/user/#{id}">#{name}</a>'
        , pug.render('a(href="/user/\\#{id}") \\#{name}', {}));
      assert.equal('<a href="/user/#{id}">ds</a>'
        , pug.render('a(href="/user/\\#{id}") #{name}', {name: 'ds'}));
    });
    ...
    it('should display error line number correctly up to token level', function() {
      ...
      assert.equal(errorLocation(str), "Pug:13");
    });
  });
  ...
  describe('.compileFile()', function () {
    ...
    it('should support caching (pass 1)', function () {
      ...
      var fn = pug.compileFile(__dirname + '/temp/input-compileFile.jade',
                                { cache: true });
  ...
  describe('.renderFile()', function () {
    it('will synchronously return a string', function () {
      var expected = fs.readFileSync(__dirname + '/cases/basic.html', 'utf8').replace(/\s/g, '');
      var actual = pug.renderFile(__dirname + '/cases/basic.jade', {name: 'foo'}).replace(/\s/g, '');
      assert(actual === expected);
    });
    it('when given a callback, it calls that rather than returning', function (done) {
      var expected = fs.readFileSync(__dirname + '/cases/basic.html', 'utf8').replace(/\s/g, '');
      pug.renderFile(__dirname + '/cases/basic.jade', {name: 'foo'}, function (err, actual) {
    ...
    it('when given a callback, it calls that rather than returning even if there are no options', function (done) {
      var expected = fs.readFileSync(__dirname + '/cases/basic.html', 'utf8').replace(/\s/g, '');
      pug.renderFile(__dirname + '/cases/basic.jade', function (err, actual) {
        if (err) return done(err);
        assert(actual.replace(/\s/g, '') === expected);
        done();
      });
    ...
    it('when given a callback, it calls that with any errors', function (done) {
      pug.renderFile(__dirname + '/fixtures/runtime.error.jade', function (err, actual) {
        assert.ok(err);
        done();
      });
    });
    it('should support caching (pass 1)', function (done) {
      ...
      pug.renderFile(__dirname + '/temp/input-renderFile.jade', { cache: true }, function (err, actual) {
        if (err) return done(err);
  ...
  describe('.compileFileClient(path, options)', function () {
    it('returns a string form of a function called `template`', function () {
      var src = pug.compileFileClient(__dirname + '/cases/basic.jade');
      var expected = fs.readFileSync(__dirname + '/cases/basic.html', 'utf8').replace(/\s/g, '');
    ...
    it('accepts the `name` option to rename the resulting function', function () {
      var src = pug.compileFileClient(__dirname + '/cases/basic.jade', {name: 'myTemplateName'});
      var expected = fs.readFileSync(__dirname + '/cases/basic.html', 'utf8').replace(/\s/g, '');
    ...
    it('should support caching (pass 1)', function () {
      ...
      var src = pug.compileFileClient(__dirname + '/temp/input-compileFileClient.jade',
                                        { name: 'myTemplateName',
                                          cache: true });
      var expected = '<div class="foo">bar</div>';
      var fn = Function('pug', src + '\nreturn myTemplateName;')(pug.runtime);
